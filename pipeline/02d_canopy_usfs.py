...
def main():
    ...
    tifs = sorted(glob.glob(str(SRC_DIR / "*_canopy2022.tif")))
    if not tifs:
        raise SystemExit(
    ...
    for tif in tifs:
        ...
        cand = glob.glob(os.path.join(os.path.dirname(tif), "*.shp"))
        if not cand:
            raise SystemExit(f"  no urban-boundary .shp beside {os.path.basename(tif)}")
