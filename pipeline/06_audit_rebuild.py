...
DIR contains the previous contracosta.geojson, census and canopy CSVs. The report
...
def ranked(doc, weights=(.55,.25,.20), source=None):
    d=pd.DataFrame([f['properties'] for f in doc['features']])
    ns={k:((d[k]-lo)/(hi-lo)).clip(0,1) for k,(lo,hi) in doc['norm'].items()}
    risk=weights[0]*(1-ns['green'])+weights[1]*(1-ns['ac'])+weights[2]*ns['pct65']
...
def main():
    ...
    old=json.loads((a.baseline/'contracosta.geojson').read_text());new=json.loads(C.OUT_FILE.read_text())
    ...
    rows=[]
    for canopy in [.35,.55,.75]:
        for ac_share in [.25,.50,.75]:
            weights=(canopy,(1-canopy)*ac_share,(1-canopy)*(1-ac_share))
    ...
    manifest={'build_date':'2026-09-07','city':C.CITY,'acs':'2020–2024 five-year, block groups','lace':'2023 tract estimates',
      ...
      'sha256':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [C.ACS_FILE,C.CENSUS_CSV,C.CANOPY_CSV,C.OUT_FILE,C.CENTERS_FILE,C.OVERLAYS_CSV,C.DATA/'cooling_directory_contracosta.json']}}
