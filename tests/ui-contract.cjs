...
const data=JSON.parse(fs.readFileSync('data/contracosta.geojson','utf8'));
...
assert.equal(data.features.filter(f=>f.properties.place==='res').length,2697);
assert.equal(data.features.filter(f=>f.properties.holc).length,0);
assert.equal(data.features.filter(f=>f.properties.place==='res'&&f.properties.green_src==='ndvi').length,431);
...
const original=JSON.parse(cp.execFileSync('git',['show','HEAD:data/contracosta.geojson'],{maxBuffer:20*1024*1024,encoding:'utf8'}));
...
const scoring={HEX:data,svals:data.norm,LIVE:live,POPW:.45,normW:()=>({heat:0,green:.55,ac:.25,age65:.20,access:0})};
